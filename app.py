...
def _get_eastmoney_hist_data(stock_code, time_module):
    ...
    for attempt in range(max_retries):
        try:
            if attempt > 0:
                print(f"  东方财富hist重试第 {attempt + 1} 次...")
            ...
            # 使用前复权数据 (qfq)
            hist_df = ak.stock_zh_a_hist(symbol=standard_code, period="daily", start_date=start_date, end_date=end_date, adjust="qfq")
            if not hist_df.empty and len(hist_df) >= 2:
                ...
                # 计算换手率：(成交量 / 流通股本) * 100
                vol = float(latest.get('成交量', 0))
                outstanding = float(latest.get('流通股本', 0))
                if outstanding > 0:
                    turnover = (vol / outstanding) * 100
                else:
                    turnover = 0.0
            ...
            if 'Connection' in error_str or 'aborted' in error_str.lower() or 'remote' in error_str.lower():
                print(f"  东方财富hist连接中断(尝试{attempt + 1}): {error_str[:80]}")
...
def judge_trade_signal(df):
    ...
    today = latest_two.iloc[1]
    yesterday = latest_two.iloc[0]
    ...
    # 卖出条件
    sell_cond1 = (today['拉升资金'] - yesterday['拉升资金']) < -0.05
    sell_cond2 = (yesterday['主力资金'] > 0) and (today['主力资金'] < 0)
...
def get_stock_data(stock_code, preferred_source=None):
    """获取单只股票的日线数据 - 使用 AkShare，带重试和备用机制
    Args:
        stock_code: 股票代码
        preferred_source: 优先使用的数据源 (None-自动选择, 'hist'-东方财富hist, 'sina'-新浪财经)
    """
    import time as time_module
    ...
    data_sources = [
        # 方法1: 使用前复权数据 (qfq)
        {"name": "前复权", "kwargs": {"adjust": "qfq"}},
        # 方法2: 使用不复权数据 (hfq)
        {"name": "不复权", "kwargs": {"adjust": "hfq"}},
    ]

    # 根据preferred_source选择数据源
    # hist-东方财富hist -> 前复权
    # sina-新浪财经 -> 前复权、不复权、后复权都尝试（新浪财经没有K线API，但所有复权选项都来自东方财富API）
    # None -> 按默认顺序尝试
    if preferred_source == 'hist':
        data_sources = data_sources[:1]  # 只用前复权
    elif preferred_source == 'sina':
        # 新浪财经：包含所有复权选项（K线数据仍来自东方财富API）
        pass
    else:
        # 其他情况保持默认
        pass

    # 日期尝试列表：优先今天，然后回退到历史日期
    date_attempts = []
    today = datetime.now()
    date_attempts.append(today.strftime('%Y%m%d'))
    ...
    for i in range(1, 8):
        ...
        if candidate.weekday() < 5:
            date_attempts.append(candidate.strftime('%Y%m%d'))

    start_date = (today - timedelta(days=DATA_LENGTH + 7)).strftime('%Y%m%d')

    # 对每个数据源进行重试
    for source in data_sources:
        # 对每个日期尝试
        for end_date in date_attempts:
            for attempt in range(2):  # 每个日期最多重试2次
                try:
                    if attempt > 0:
                    ...
                    print(f"  尝试获取{stock_code}数据 - {source['name']} (截止日期:{end_date})(尝试{attempt + 1})")
                    # 使用 stock_zh_a_hist 获取单个股票数据
                    ...
                    if df.empty:
                        print(f"  {source['name']} (截止:{end_date}) 数据为空")
                        break  # 该日期数据为空，尝试下一日期

                    if len(df) < 2:
                        print(f"  {source['name']} (截止:{end_date}) 数据不足2天")
                        break  # 数据不足，尝试下一日期

                    # 使用列索引避免中文编码问题
                    # 列顺序：日期, 股票代码, 开盘, 收盘, 最高, 最低, 成交量, 成交额, 振幅, 涨跌幅, 涨跌额, 换手率
                    df_new = pd.DataFrame()
                    df_new['trade_date'] = df.iloc[:, 0]  # 日期
                    df_new['open'] = df.iloc[:, 2]  # 开盘
                    df_new['close'] = df.iloc[:, 3]  # 收盘
                    df_new['high'] = df.iloc[:, 4]  # 最高
                    df_new['low'] = df.iloc[:, 5]  # 最低
                    df_new['vol'] = df.iloc[:, 6]  # 成交量
                    ...
                    print(f"  获取{stock_code}数据成功 - {source['name']}，共{len(df_new)}条，截止{end_date}")
                    return df_new
                    ...
                    if is_connection_error:
                        print(f"  {source['name']} 连接中断(尝试{attempt + 1}): {error_str[:60]}")
                        if attempt == 1:  # 最后一次重试也失败
                            print(f"  {source['name']} 连续失败，切换数据源")
                            break  # 切换到下一数据源
                    else:
                        print(f"  {source['name']} 获取失败: {error_str[:60]}")
                        break  # 非连接错误，直接切换数据源

    # 所有数据源都失败
    print(f"获取股票{stock_code}数据失败：所有数据源均失败")
    return pd.DataFrame()
...
def save_email_record(stock_code, stock_name, signal_type, price, condition, content):
    ...
    # 将今日邮件移到昨日
    email_data["emails"][standard_code]["yesterday"] = email_data["emails"][standard_code].get("today")
...
def analyze_current_price():
    ...
    try:
        ...
        # 使用标准6位代码（不带sh/sz前缀）
        df = ak.stock_zh_a_hist(symbol=to_standard_code(stock_code), period="daily",
...
def get_stocks():
    ...
    for stock in stocks_data["stocks"]:
        code = stock["code"]
        if code in email_data["emails"]:
            stock["emails"] = email_data["emails"][code]
        else:
